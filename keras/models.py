...
class JointEmbeddingModel:
    def __init__(self, config):
        ...
        self.methname = Input(shape=(self.data_params['methname_len'],), dtype='int32',
                              name='i_methname')
        ...
        self.desc_good = Input(shape=(self.data_params['desc_len'],), dtype='int32',
                               name='i_desc_good')
        self.desc_bad = Input(shape=(self.data_params['desc_len'],), dtype='int32',
                              name='i_desc_bad')
    ...
    def build(self):
        ...
        maxpool = Lambda(lambda x: K.max(x, axis=1, keepdims=False),
                         output_shape=lambda x: (x[0], x[2]), name='maxpool_methname')
        methname_pool = Concatenate(name='concat_methname_lstms')(
            [maxpool(methname_f_dropout), maxpool(methname_b_dropout)])
        ...
        # 2.rnn
        f_rnn = LSTM(self.model_params.get('n_lstm_dims', 100), return_sequences=True,
                     recurrent_dropout=0.2,
                     name='lstm_apiseq_f')
        b_rnn = LSTM(self.model_params.get('n_lstm_dims', 100), return_sequences=True,
                     recurrent_dropout=0.2,
        ...
        # 3.maxpooling
        maxpool = Lambda(lambda x: K.max(x, axis=1, keepdims=False),
                         output_shape=lambda x: (x[0], x[2]), name='maxpool_apiseq')
        apiseq_pool = Concatenate(name='concat_apiseq_lstms')(
            [maxpool(apiseq_f_dropout), maxpool(apiseq_b_dropout)])
        ...
        # 4.maxpooling
        maxpool = Lambda(lambda x: K.max(x, axis=1, keepdims=False),
                         output_shape=lambda x: (x[0], x[2]), name='maxpool_tokens')
        ...
        code_repr = Dense(self.model_params.get('n_hidden', 400), activation='tanh',
                          name='dense_coderepr')(merged_code_repr)

        self._code_repr_model = Model(inputs=[methname, apiseq, tokens], outputs=[code_repr],
        ...
        # 3. maxpooling
        maxpool = Lambda(lambda x: K.max(x, axis=1, keepdims=False),
                         output_shape=lambda x: (x[0], x[2]), name='maxpool_desc')
        desc_pool = Concatenate(name='concat_desc_rnns')(
            [maxpool(desc_f_dropout), maxpool(desc_b_dropout)])
        ...
        sim_model = Model(inputs=[methname, apiseq, tokens, desc], outputs=[cos_sim],
                          name='sim_model')
        ...
        '''
        good_sim = sim_model(
            [self.methname, self.apiseq, self.tokens, self.desc_good])  # similarity of good output
        bad_sim = sim_model(
            [self.methname, self.apiseq, self.tokens, self.desc_bad])  # similarity of bad output
        ...
        self._training_model = Model(
            inputs=[self.methname, self.apiseq, self.tokens, self.desc_good, self.desc_bad],
    ...
    def compile(self, optimizer, **kwargs):
        ...
        self._training_model.compile(loss=lambda y_true, y_pred: y_pred + y_true - y_true,
