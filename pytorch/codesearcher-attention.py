...
from configs_attention import get_config
from data import load_dict, load_vecs, save_vecs, CodeSearcherNumpyDataSet
from models_attention import JointEmbeder
    ...
    def train(self, model):
        ...
        for epoch in range(self.conf['reload'] + 1, nb_epoch):
            ...
            for names, apis, toks, good_descs, bad_descs in data_loader:
                ...
                optimizer.step()
                if itr % log_every == 0:
                    tensorboard_writer.add_scalar("attention_loss", np.mean(losses),
                                                  epoch * 10 + itr // 100)
                    logger.info(
                ...
                if epoch and epoch % valid_every == 0:
                    ...
                    tensorboard_writer.add_scalar("attention_acc1", acc1, epoch)
                    tensorboard_writer.add_scalar("attention_mrr", mrr, epoch)
                    tensorboard_writer.add_scalar("attention_map", map, epoch)
                    tensorboard_writer.add_scalar("attention_ndcg", ndcg, epoch)
