...
class SeqEncoder(nn.Module):
    ...
    def forward(self, input, input_lengths=None):
        embedded = self.embedding(
            input)  # input: [batch_sz x seq_len]  embedded: [batch_sz x seq_len x emb_sz]
...
class JointEmbeder(nn.Module):
    def __init__(self, config):
        ...
        if not os.path.exists(config['workdir'] + 'models_attention/'):
            os.makedirs(config['workdir'] + 'models_attention/')
